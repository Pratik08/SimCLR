...
def perform_predict(estimator, input_fn, checkpoint_path=None):
    ...
    EVAL_PREDS_OUT = '/home/psd2120/research/data/finetune_eval1_preds.csv'

    with open(EVAL_PREDS_OUT, 'a+') as f:
        writer = csv.writer(f)
        for item in est_output:
            writer.writerow([item['label']] + list(item['top_5']))
...
def main(argv):
    ...
    if FLAGS.mode == 'eval':
        ...
        estimator.train(data_lib.build_input_fn(num_classes, True),
