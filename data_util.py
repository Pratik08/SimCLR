...
def random_apply(func, p, x):
    ...
    return tf.cond(tf.less(tf.random_uniform([], minval=0, maxval=1,
                                             dtype=tf.float32),
...
def color_jitter(image, strength, random_order=True):
    ...
    if random_order:
        ...
        return color_jitter_nonrand(image, brightness, contrast, saturation,
                                    hue)
    ...
    with tf.name_scope('distort_color'):
        def apply_transform(i, x, brightness, contrast, saturation, hue):
            ...
            if brightness != 0 and i == 0:
                ...
                x = tf.image.random_saturation(x, lower=1-saturation,
                                               upper=1+saturation)
            elif hue != 0:
        ...
        def apply_transform(i, x):
            ...
            def saturation_foo():
                if saturation == 0:
                    ...
                    return tf.image.random_saturation(x, lower=1-saturation,
                                                      upper=1+saturation)
            ...
            x = tf.cond(tf.less(i, 2),
                        lambda: tf.cond(tf.less(i, 1), brightness_foo,
                                        contrast_foo),
                        lambda: tf.cond(tf.less(i, 3),
                                        saturation_foo, hue_foo))
            return x
...
def gaussian_blur(image, kernel_size, sigma, padding='SAME'):
    ...
    blurred = tf.nn.depthwise_conv2d(image, blur_h, strides=[1, 1, 1, 1],
                                     padding=padding)
    blurred = tf.nn.depthwise_conv2d(blurred, blur_v, strides=[1, 1, 1, 1],
                                     padding=padding)
    if expand_batch_dim:
...
def batch_random_blur(images_list, height, width, blur_probability=0.5):
    ...
    def generate_selector(p, bsz):
        shape = [bsz, 1, 1, 1]
        selector = tf.cast(tf.less(tf.random_uniform(shape, 0, 1,
                                                     dtype=tf.float32), p),
                           tf.float32)
        return selector
