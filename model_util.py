...
def add_weight_decay(adjust_per_optimizer=True):
    ...
    tf.losses.add_loss(FLAGS.weight_decay * tf.add_n(l2_losses),
...
def linear_layer(x,
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        if use_bn:
            x = resnet.batch_norm_relu(x, is_training, relu=False,
                                       center=use_bias)
...
def projection_head(hiddens, is_training, name='head_contrastive'):
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        if FLAGS.head_proj_mode == 'none':
            ...
            hiddens = linear_layer(hiddens, is_training, FLAGS.head_proj_dim,
            ...
            for j in range(1, FLAGS.num_nlh_layers + 1):
                ...
                hiddens = linear_layer(hiddens, is_training,
                                       FLAGS.head_proj_dim, use_bias=False,
                                       use_bn=True, name='nl_%d' %j)
        else:
